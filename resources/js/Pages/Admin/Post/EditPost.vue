        ...
        <div class="allCreatePost">
            <div class="allPostPanelTop">
                ...
                <div class="allPostTitle">
                    ...
                    <span>/</span>
                    <inertia-link :href="'/admin/post/' + posts.id + '/edit'">{{posts.title}}</inertia-link>
                </div>
            </div>
                    ...
                    <div class="allCreatePostDetail">
                        ...
                        <transition name="slide-fade">
                            <div class="allCreatePostDetailItems">
                                ...
                                <div class="allCreatePostDetailItem">
                                    <label>سایز :</label>
                                    <post-taxonami :taxes="sizes" :taxRoute="'سایز'" :tax="size"  v-on:sendTax="getSize"></post-taxonami>
                                </div>
                                <div class="allCreatePostDetailItem">
                                    <label>رنگ :</label>
                                    <post-taxonami :taxes="colors" :taxRoute="'رنگ'" :tax="color"  v-on:sendTax="getColor"></post-taxonami>
                                </div>
                                <div class="allCreatePostDetailItem">
                                    <label>برند :</label>
                                    <post-taxonami :taxes="brands" :taxRoute="'برند'" :tax="brand"  v-on:sendTax="getBrand"></post-taxonami>
                                </div>
                                <div class="allCreatePostDetailItem">
                                    <label>زمان :</label>
                                    <post-taxonami :taxes="times" :taxRoute="'زمان'" :tax="time"  v-on:sendTax="getTime"></post-taxonami>
                                </div>
                            </div>
                        </transition>
                    </div>
...
export default {
    props: ['errors','posts','categories','brands','times','time','colors','sizes','category','brand','color','size'],
    components: { AdminLayout, SvgIcon, PostTaxonami,CKEditor: CKEditor.component, ShowImage },
    metaInfo: {
      title: 'ویرایش پست',
        link: [
    ...
    data(){
        return{
            ...
            i : 0,
            form:{
                guarantee : this.posts.post_meta[0].guarantee,
                summery : this.posts.summery,
                price : this.posts.offPrice,
                count : this.posts.count,
                slug : this.posts.slug,
                status : this.posts.status,
                title : this.posts.title,
                titleEn : this.posts.post_meta[0].titleEn,
                body : this.posts.post_meta[0].body,
                ...
                allBrand: null,
                allTime: null,
                abilities: JSON.parse(this.posts.post_meta[0].ability),
                rates: JSON.parse(this.posts.post_meta[0].rate),
                properties:JSON.parse(this.posts.post_meta[0].property),
                image: [],
                allAbility: [],
                allRate: [],
                allProperty: [],
